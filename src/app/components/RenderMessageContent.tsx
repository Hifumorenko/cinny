...
import { parseRedditPostUrl, testRedditPostUrl } from '../plugins/reddit';
...
export function RenderMessageContent({
  ...
  const renderUrlsPreview = (urls: string[]) => {
    ...
    const redditUrls = filteredUrls.filter(testRedditPostUrl);
    ...
    const spoiledRedditIds = new Set(
      Array.from(spoiledUrls, (spoiledUrl) => parseRedditPostUrl(spoiledUrl)?.id).filter(
        (id) => id !== undefined
      )
    );
      ...
      <>
        ...
        {redditUrls.map((url) => (
          <RedditPreviewCard
            ...
            url={url}
            ts={ts}
            spoiler={spoiledRedditIds.has(parseRedditPostUrl(url)?.id ?? '')}
          />
        ))}
